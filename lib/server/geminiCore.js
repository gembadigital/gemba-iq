...
//
// Item: "Teklif oluşturma tablo dönüştürme API hatası" — the underlying bug
// was that this function had NO overall time budget. All api/gemini/*.js
// routes are capped at maxDuration: 60s (vercel.json). A single slow Gemini
// call (e.g. converting a large pasted table, or the model just being slow
// that moment) combined with the retry backoff above (up to ~8.4s of pure
// waiting across 3 retries) could push the total past 60s. When that
// happens, VERCEL kills the whole function itself — this bypasses every
// try/catch in this file entirely, so the caller never gets our normal
// { status: 500, body: { error: "..." } } JSON; it just gets a raw platform
// timeout with no useful message, which is what showed up as a generic "API
// hatası" in the UI. Confirmed via Vercel runtime error logs: "Vercel
// Runtime Timeout Error: Task timed out after 60 seconds" on
// /api/gemini/[...action].
//
// Fix: keep an overall deadline comfortably under the 60s ceiling. Each
// attempt races against the remaining budget, and if we're going to run out
// of time (for the call itself or for another retry's backoff), we throw
// our own clear, translated error immediately instead of letting Vercel's
// hard kill fire. This guarantees the frontend always gets a real JSON
// error message instead of a mysterious timeout.
const GEMINI_DEADLINE_MS = 48000; // 12s safety margin under the 60s function cap
const GEMINI_TIMEOUT_MESSAGE =
  "Yapay zeka yanıtı çok uzun sürdü (zaman aşımı). Lütfen daha kısa/az içerikle tekrar deneyin.";
...
export async function generateWithRetry(params, maxRetries = 3, baseDelayMs = 1200) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    const remaining = GEMINI_DEADLINE_MS - (Date.now() - startedAt);
    if (remaining <= 2000) {
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error("GEMINI_CALL_TIMEOUT")), remaining);
      });
      ...
      const isTimeout = errText === "GEMINI_CALL_TIMEOUT";
      const isTransient = isTimeout || /UNAVAILABLE|high demand|503|overloaded/i.test(errText);
      if (isTimeout) {
        // Out of time entirely — no point retrying, we're already at budget.
        throw new Error(GEMINI_TIMEOUT_MESSAGE);
      }
      if (!isTransient || attempt === maxRetries) {
        throw err;
      }
      const remainingForBackoff = GEMINI_DEADLINE_MS - (Date.now() - startedAt) - 2000;
      const delay = Math.min(baseDelayMs * Math.pow(2, attempt), Math.max(remainingForBackoff, 0));
      if (delay <= 0) {
        throw new Error(GEMINI_TIMEOUT_MESSAGE);
      }
...
// Robustly parses a JSON object out of a raw Gemini text response.
//
// Item: "Hizmet kataloğu: Tablo dönüştürme hatası: Unexpected non-whitespace
// character after JSON at position 6281 (line 4 column 1)". Even with
// responseMimeType: "application/json", Gemini sometimes appends trailing
// content after the closing "}" — a stray newline + commentary, a repeated/
// duplicated JSON block, or leftover markdown fencing remnants. A plain
// JSON.parse(resultText) (and the ```json-stripping fallback that used to be
// the only other attempt) both fail outright the moment ANY non-whitespace
// character exists after the first complete JSON value, even though that
// first value is perfectly valid. This helper walks the string with a
// brace/string/escape-aware scanner to find the first complete top-level
// JSON object and parses only that slice, ignoring whatever the model tacked
// on afterward. Shared by every Gemini action that expects a single JSON
// object back (assist, sales-coach, convert-table, generate-custom-pitch),
// so this class of bug can't resurface in one endpoint while being fixed in
// another.
...
function parseGeminiJsonResponse(resultText) {
  try {
    ...
    try {
      ...
      const extracted = extractFirstJsonObject(cleanJsonStr);
      if (!extracted) {
        throw new Error("Yapay zekadan geçerli bir JSON yanıtı alınamadı.");
      }
...
Preserve the existing placeholders like {{FirstName}}, {{LastName}}, {{Company}}, or {{Department}}. Structure your entire response as a JSON object with strictly two keys: "subject" and "body". Return ONLY the raw parseable JSON string - do NOT wrap in markdown.`;
    ...
    const parsedResult = parseGeminiJsonResponse(resultText);
...
// Pazarlama & İş Geliştirme modülü — Dijital Pazarlama Zekası sekmesi.
// Kullanıcının yüklediği bir rapor metnini (GA4/Search Console export'u,
// SEMrush/Ahrefs raporu, veya herhangi bir pazar/trend raporu — PDF/Excel'den
// çıkarılmış düz metin olarak) analiz eder ve eksik anahtar kelime fırsatları,
// önerilen blog konuları, ve somut pazarlama aksiyonları üretir.
export async function runGeminiMarketingReportAnalysis({ reportText, fileName, sourceType, industryContext }) {
  if (!process.env.GEMINI_API_KEY) {
    return {
      status: 400,
      body: { error: "GEMINI_API_KEY is not configured in environment variables. Please supply it under Settings > Secrets." },
  ...
  if (!reportText || !String(reportText).trim()) {
    return { status: 400, body: { error: "Analiz edilecek rapor içeriği boş." } };
  }

  try {
    const systemInstruction = `You are a B2B industrial/consulting marketing strategist analyzing a raw marketing/analytics report (Google Analytics, Search Console, SEMrush, Ahrefs, or a similar trend/traffic report exported as text) for a Lean/OPEX/quality management consulting firm ("Gemba Partner") operating in Turkey. The company sells operational excellence consulting (Lean, Six Sigma, quality systems) to manufacturing/industrial companies.

Given the raw report text below, extract or infer:
1. keywords: an array of 8-15 concrete SEO/content keyword opportunities (in Turkish, industry-relevant, e.g. "yalın üretim danışmanlığı", "OEE artırma yöntemleri") that this report suggests are underserved or worth targeting.
2. blogTopics: an array of 6-10 specific blog post title ideas (in Turkish) that would target those keywords and this audience.
3. strategyActions: an array of 5-8 concrete, actionable marketing strategy recommendations (in Turkish) derived from the report's data (e.g. traffic drops, high-bounce pages, competitor gaps, seasonal trends).
4. rawSummary: a 2-4 sentence plain-Turkish executive summary of what the report shows.

Respond with ONLY a raw parseable JSON object with exactly these four keys. No markdown fencing, no commentary.`;

    const userContent = `Rapor dosya adı: ${fileName || "Bilinmiyor"}\nRapor kaynağı: ${sourceType || "Belirtilmemiş"}\nSektör bağlamı: ${industryContext || "Genel"}\n\nRapor içeriği:\n${String(reportText).slice(0, 30000)}`;
    ...
    return {
      ...
      body: {
        ...
        blogTopics: Array.isArray(parsedResult.blogTopics) ? parsedResult.blogTopics : [],
        strategyActions: Array.isArray(parsedResult.strategyActions) ? parsedResult.strategyActions : [],
        rawSummary: parsedResult.rawSummary || "",
      },
    };
  } catch (error) {
    console.error("Gemini Marketing Report Analysis error:", error);
    return { status: 500, body: { error: error.message || "Rapor analizi Gemini ile tamamlanamadı." } };
  }
}
