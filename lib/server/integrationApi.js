...
// Kullanıcı hatası (Gemba Sales Manager Agent üzerinden): ajan bir fırsat
// yazarken "pipeline" alanına serbest metin olarak "İş Geliştirme
// Pipeline'ı" yazmış — bu Gemba IQ'da tanımlı bir pipeline değil (arayüz
// sadece "Sales Pipeline Standard"ı tanıyor; "İş Geliştirme Pipeline'ı"
// aslında tamamen ayrı bir veri modeli — Marketing & Business Development
// modülündeki Target Account.bdPipelineStage, Deal değil). Kök neden:
// integrationDealCreate/Update, "priority" alanı için zaten yaptığı gibi
// (bkz. aşağıdaki allowlist) "pipeline"/"stage" alanlarını HİÇ
// doğrulamadan doğrudan kaydediyordu — aynı "backend hiçbir şeyi
// doğrulamıyor" ailesinden bir bug (leads için write endpoint olmaması,
// query= parametresinin filtrelememesi gibi). Sonuç: kayıt DB'ye yazılıyor
// ve ham liste sorgusunda görünüyor, ama panel Kanban görünümü fırsatları
// KESİN eşleşmeyle (d.stage === stage) sütunlara diziyor — tanımsız bir
// stage/pipeline değeri hiçbir sütunla eşleşmediği için fırsat hiçbir
// Kanban sütununda görünmüyor (kullanıcı tarafında "kayboldu"/"404" gibi
// alınıyor). Çözüm: pipeline tek bir sabit değer olduğundan allowlist ile
// sınırlandırıldı; stage ise organizasyonun panelde GERÇEKTEN tanımlı
// (varsayılan veya yeniden adlandırılmış) aşama listesine göre doğrulanıp
// eşleşmiyorsa ilk aşamaya düşürülüyor (Marketing Hub'daki BD Pipeline'ın
// zaten sahip olduğu savunmacı fallback deseniyle aynı).
const SALES_PIPELINE_NAME = "Sales Pipeline Standard";

// DealManagementView.tsx'teki DEFAULT_STAGES ile BİREBİR aynı tutulmalı —
// biri değişirse diğeri de değişmeli. Organizasyon panelden aşamaları
// yeniden adlandırmış/özelleştirmişse (bkz. getAuxiliaryRow), o liste
// öncelikli kullanılır; bu sadece hiç özelleştirme yapılmamış organizasyonlar
// için varsayılandır.
...
async function getOrgDealStages(adminClient, organizationId) {
  const auxiliary = await getAuxiliaryRow(adminClient, organizationId);
  const saved = Array.isArray(auxiliary?.pipelineStages) ? auxiliary.pipelineStages : null;
  return saved && saved.length > 0 ? saved : DEFAULT_DEAL_STAGES;
}
...
export async function integrationDealCreate(request, response) {
  ...
  let validStages;
  try {
    validStages = await getOrgDealStages(adminClient, credential.organization_id);
  } catch (err) {
  ...
  const dealData = {
    ...
    stage: validStages.includes(body.stage) ? body.stage : validStages[0],
    owner: body.owner || credential.name,
    pipeline: body.pipeline === SALES_PIPELINE_NAME ? body.pipeline : SALES_PIPELINE_NAME,
  ...
  // Ajanın gönderdiği pipeline/stage geçersizse sessizce normalize edildi
  // (aşağıda) — çağıran taraf bunu fark edebilsin diye ayrıca bildiriyoruz,
  // aksi halde ajan "İş Geliştirme Pipeline'ı" gibi geçersiz bir değeri
  // başarıyla kaydettiğini sanıp aynı hatayı tekrar edebilir.
  const warnings = [];
  if (body.pipeline && body.pipeline !== dealData.pipeline) {
    warnings.push(
      `'${body.pipeline}' is not a recognized pipeline in Gemba IQ — only '${SALES_PIPELINE_NAME}' exists. The deal was saved with pipeline='${dealData.pipeline}'.`
    );
  }
  if (body.stage && body.stage !== dealData.stage) {
    warnings.push(
      `'${body.stage}' is not a stage configured on this organization's Sales Pipeline board (valid: ${validStages.join(", ")}). The deal was saved with stage='${dealData.stage}'.`
    );
  }

  return response.status(201).json({ created: true, id: dealId, data: dealData, ...(warnings.length ? { warnings } : {}) });
}
...
export async function integrationDealUpdate(request, response) {
  ...
  // create ile aynı kök nedenden (bkz. yukarıdaki not): update de body'yi
  // hiç doğrulamadan doğrudan merge ediyordu — bir agent geçersiz bir
  // pipeline/stage göndererek var olan sağlıklı bir fırsatı da bu şekilde
  // "kaybolur" hale getirebilirdi.
  if (Object.prototype.hasOwnProperty.call(body, "pipeline") && body.pipeline !== SALES_PIPELINE_NAME) {
    merged.pipeline = SALES_PIPELINE_NAME;
    warnings.push(
      `'${body.pipeline}' is not a recognized pipeline in Gemba IQ — only '${SALES_PIPELINE_NAME}' exists. pipeline was left unchanged as '${SALES_PIPELINE_NAME}'.`
    );
  }
  if (Object.prototype.hasOwnProperty.call(body, "stage")) {
    ...
    if (!validStages.includes(body.stage)) {
      merged.stage = row.data?.stage && validStages.includes(row.data.stage) ? row.data.stage : validStages[0];
      warnings.push(
        `'${body.stage}' is not a stage configured on this organization's Sales Pipeline board (valid: ${validStages.join(", ")}). stage was left unchanged as '${merged.stage}'.`
      );
    }
  }
  ...
  return response.status(200).json({ updated: true, id, data: merged, ...(warnings.length ? { warnings } : {}) });
}
