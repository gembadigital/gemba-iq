...
// ProposalLetterheadBody now lives in its own file (./ProposalLetterheadBody)
// so the exact same styled A4 letterhead HTML/CSS document used here for the
// "PDF İndir" download / e-mail PDF attachment can also be reused by the
// Fırsat (Deal) drawer's embedded proposal panel — see
// OpportunityDrawerExtension.tsx's ProposalContractSection, which previously
// had its own separate, hand-drawn jsPDF renderer with no knowledge of the
// proposal's real content, letterhead colors, or uploaded cover/page images
// (the same bug class fixed here earlier: "pdf görüntüsü yeşil başka bir
// şablon çıkıyor").
...
export default function ProposalManagementView() {
  ...
  // Opens the matching proposal's detail panel when navigated here via the
  // shared "crm-navigate" event with a proposal id — used both by
