...
// Utility function to generate high-fidelity PDF Blob URLs on the fly
function generateProposalPdfBlobUrl(prop: Proposal, lang: string): string {
  try {
    const doc = new jsPDF({
    ...
    // Standard friendly conversion to avoid PDF encoding issues in default Helvetica
    ...
    doc.setTextColor(textLight[0], textLight[1], textLight[2]);
    doc.text(
      safeStr(`${isTR ? "Musteri" : "Client"}: ${prop.companyName}  |  ${isTR ? "Ilgili Kisi" : "Attn"}: ${prop.contactPerson}`),
      marginX + 5,
      y + 15
    );
    doc.text(
      safeStr(`${isTR ? "Sorumlu Danisman" : "Partner"}: ${prop.owner}  |  ${isTR ? "Durum" : "Status"}: ${prop.status}`),
      marginX + 5,
      y + 20
    );
    ...
    const descText = prop.description || (isTR ? "Musteri sahasindaki israflarin tespiti, VSM analizi ve deger akisi optimizasyonu surec tasarimi." : "Continuous improvement workshop focused on shopfloor wastes reduction.");
    ...
    if (servicesList.length === 0) {
      ...
      servicesList.forEach((s) => {
        doc.text(`[x] ${safeStr(s)}`, marginX + 4, y);
        y += 4.5;
      });
    }

    // --- 3. Option Packages and Detailed Budgets ---
    y = y + 4;
    doc.setFont("Helvetica", "bold");
    doc.setFontSize(9.5);
    doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
    ...
    doc.text(safeStr(isTR ? `${prop.companyName} Yetkili Temsilcisi` : `Authorized ${prop.companyName} Representative`), marginX + 115, y + 4);

    // Stamp Confidentiality Notice
    doc.setFont("Helvetica", "italic");
    doc.setFontSize(7);
    ...
    return typeof out === "string" ? out : (out as any).toString();
  } catch (err) {
