...
// Haritası" ve "İş Geliştirme Pipeline'ı" sekmeleri AYRI bir hedef firma
...
export default function MarketingHubView({ onNavigateToTab }: MarketingHubViewProps) {
  const { t } = useLanguage();
  ...
  const StatCard = ({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: string }) => (
    <div className="bg-white dark:bg-[#1b1a19] p-4 rounded border border-[#EDEBE9] dark:border-[#323130] shadow-sm">
      ...
      <div className={`text-xl font-bold mt-1 ${accent || "text-[#0078D4]"}`}>{value}</div>
      {sub && <p className="text-[10px] text-slate-450 mt-1">{sub}</p>}
    </div>
  );

  const subTabs: { key: SubTab; label: string; icon: React.ReactNode }[] = [
    { key: "overview", label: "Marketing Hub Overview", icon: <LayoutDashboard className="w-4 h-4 flex-shrink-0" /> },
    { key: "industry-intel", label: "Industry Intelligence", icon: <TrendingUp className="w-4 h-4 flex-shrink-0" /> },
    { key: "target-market", label: "Target Market & Competitor Map", icon: <Target className="w-4 h-4 flex-shrink-0" /> },
    { key: "bd-pipeline", label: "Business Development Pipeline", icon: <Briefcase className="w-4 h-4 flex-shrink-0" /> },
    { key: "playbooks", label: "Industry Playbooks", icon: <BookOpen className="w-4 h-4 flex-shrink-0" /> },
    { key: "growth-health", label: "Growth Health", icon: <BarChart2 className="w-4 h-4 flex-shrink-0" /> },
    { key: "digital-intel", label: "Digital Marketing Intelligence", icon: <Search className="w-4 h-4 flex-shrink-0" /> },
    { key: "kpi-okr", label: "BD KPIs, Win/Loss & OKR", icon: <Award className="w-4 h-4 flex-shrink-0" /> },
  ];
  ...
  return (
    <div id="marketing-hub-view-root" className="space-y-6">
      {toast && (
        <div
      ...
      <div className="bg-white dark:bg-[#1b1a19] p-6 border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm relative overflow-hidden">
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-purple-500 via-[#0078D4] to-emerald-500" />
        <div className="flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-[#0078D4]" />
          <h2 className="text-base font-bold text-slate-800 dark:text-slate-100">{t("Marketing & Business Development")}</h2>
        </div>
        <p className="text-xs text-slate-500 mt-1 max-w-3xl">
          {t(
            "Manage sector intelligence, target market mapping, business development pipeline, industry playbooks, growth health, digital marketing intelligence, and strategic KPIs & OKRs in one place."
          )}
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        <aside className="lg:col-span-3 bg-white dark:bg-[#1b1a19] border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm p-2 space-y-1 h-fit lg:sticky lg:top-4">
          {subTabs.map((tabDef) => (
            <button
              key={tabDef.key}
              type="button"
              onClick={() => setActiveSubTab(tabDef.key)}
              className={`w-full flex items-center gap-2 text-xs font-semibold px-3 py-2.5 rounded transition-all cursor-pointer ${
                activeSubTab === tabDef.key
                  ? "bg-[#EAF2FF] dark:bg-[#1E3A5F]/20 text-[#0078D4] border-l-4 border-l-[#0078D4]"
                  : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-[#252423] border-l-4 border-l-transparent"
              }`}
            >
              {tabDef.icon}
              <span className="text-left">{t(tabDef.label)}</span>
            </button>
          ))}
        </aside>
        ...
        <div className="lg:col-span-9 space-y-4">
          {activeSubTab === "overview" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatCard label={t("Sectors Tracked")} value={String(industryStats.length)} sub={t("Industry intelligence")} />
                <StatCard label={t("Target Companies")} value={String(accounts.length)} sub={t("In target market registry")} />
                <StatCard
                ...
                />
                <StatCard label={t("Overall Win Rate")} value={`%${lossAnalysis.winRate.toFixed(0)}`} sub={t("Won vs Lost")} accent="text-emerald-600" />
                <StatCard label={t("Active Playbooks")} value={String(playbooks.length)} sub={t("Industry playbooks")} />
                <StatCard label={t("Open Pipeline Value")} value={formatCurrencyShort(growthHealth.pipelineValue)} sub={t("Open opportunities")} />
                <StatCard label={t("Active OKRs")} value={String(goals.filter((g) => g.status === "Devam Ediyor").length)} sub={t("Strategic goals in progress")} />
              </div>
              <div className="bg-white dark:bg-[#1b1a19] p-5 border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm">
                <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase mb-3">{t("Top Sectors by Strength")}</h3>
                <div className="space-y-2">
                  {industryStats.slice(0, 5).map((row) => (
                    <div key={row.industry} className="flex items-center justify-between text-xs border-b border-[#EDEBE9] dark:border-[#323130] pb-2">
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{row.industry}</span>
                      <span className="text-slate-500">
                        {row.customerCount} {t("customers")} · %{row.winRate.toFixed(0)} {t("win rate")}
                      </span>
                    </div>
                  ))}
                  {industryStats.length === 0 && <p className="text-xs text-slate-400">{t("No data yet.")}</p>}
                </div>
              </div>
            </div>
          )}
          ...
          {activeSubTab === "industry-intel" && (
            <div className="bg-white dark:bg-[#1b1a19] border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm overflow-hidden">
              <div className="p-4 border-b border-[#EDEBE9] dark:border-[#323130]">
                <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase">{t("Industry Intelligence")}</h3>
                <p className="text-[11px] text-slate-500 mt-1">
                  {t("Automatically computed from your existing customer and deal data — no manual entry required.")}
                </p>
              </div>
          ...
          {activeSubTab === "target-market" && (
            <div className="space-y-4">
              <div className="bg-white dark:bg-[#1b1a19] p-4 border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-3">
              ...
              {showAddTarget && (
                <form onSubmit={handleAddTargetCompany} className="bg-white dark:bg-[#1b1a19] border border-[#0078D4]/20 rounded p-5 shadow-md space-y-3">
              ...
              {reviewPendingAccounts.length > 0 && (
                <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded p-4">
              ...
              <div className="space-y-3">
                {filteredAccounts.map((account) => {
                  ...
                  return (
                    <div key={account.id} className="bg-white dark:bg-[#1b1a19] border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm overflow-hidden">
                      <div
                ...
                {accounts.length === 0 && (
                  <div className="bg-white dark:bg-[#1b1a19] border border-[#EDEBE9] dark:border-[#323130] rounded p-8 text-center text-xs text-slate-400">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-3 overflow-x-auto pb-2">
                {BD_PIPELINE_STAGES.map((stage) => (
                  <div key={stage} className="flex-shrink-0 w-64 bg-[#FAF9F8] dark:bg-[#201f1e] border border-[#EDEBE9] dark:border-[#323130] rounded">
          ...
          {activeSubTab === "playbooks" && (
            <div className="space-y-4">
              ...
              {showAddPlaybook && (
                <form onSubmit={handleSavePlaybook} className="bg-white dark:bg-[#1b1a19] border border-[#0078D4]/20 rounded p-5 shadow-md space-y-3 text-xs">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase block mb-1">{t("Industry / Sector *")}</label>
                      <input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {playbooks
                  ...
                  .map((p) => (
                    <div key={p.id} className="bg-white dark:bg-[#1b1a19] border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm p-4 space-y-2">
                ...
                {playbooks.length === 0 && (
                  <div className="md:col-span-2 bg-white dark:bg-[#1b1a19] border border-[#EDEBE9] dark:border-[#323130] rounded p-8 text-center text-xs text-slate-400">
          ...
          {activeSubTab === "growth-health" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <StatCard label={t("Active Customers")} value={String(growthHealth.activeCustomers)} />
                <StatCard label={t("Won Deals")} value={String(growthHealth.wonCount)} accent="text-emerald-600" />
                <StatCard label={t("Lost Deals")} value={String(growthHealth.lostCount)} accent="text-rose-600" />
                <StatCard label={t("Open Pipeline Value")} value={formatCurrencyShort(growthHealth.pipelineValue)} />
                <StatCard label={t("Avg Sales Cycle")} value={`${growthHealth.avgCycleDays.toFixed(0)} ${t("days")}`} />
                <StatCard label={t("At-Risk Customers")} value={String(growthHealth.riskyCustomers.length)} accent="text-amber-600" sub={t("Health score below 50")} />
              </div>
              <div className="bg-white dark:bg-[#1b1a19] p-5 border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm">
                <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase mb-3">{t("Open Pipeline Value by Stage")}</h3>
                <div className="space-y-2">
                  {Object.entries(growthHealth.stageValueMap)
                    .sort((a, b) => b[1] - a[1])
                    .map(([stage, value]) => (
                      <div key={stage} className="flex items-center justify-between text-xs border-b border-[#EDEBE9] dark:border-[#323130] pb-2">
                        <span className="font-semibold text-slate-700 dark:text-slate-200">{stage}</span>
                        <span className="text-slate-500">{formatCurrencyShort(value)}</span>
                      </div>
                    ))}
                  {Object.keys(growthHealth.stageValueMap).length === 0 && <p className="text-xs text-slate-400">{t("No open pipeline.")}</p>}
                </div>
              </div>
          ...
          {activeSubTab === "digital-intel" && (
            <div className="space-y-4">
              <div className="bg-white dark:bg-[#1b1a19] p-5 border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm space-y-3">
                <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase">{t("Digital Marketing Intelligence")}</h3>
                <p className="text-[11px] text-slate-500">
                  {t(
              ...
              {analysisResult && (
                <div className="bg-white dark:bg-[#1b1a19] border border-emerald-200 dark:border-emerald-900 rounded shadow-sm p-5 space-y-4">
              ...
              {reportInsights.length > 0 && (
                <div className="space-y-2">
                  ...
                  {reportInsights.map((ins) => (
                    <div key={ins.id} className="bg-white dark:bg-[#1b1a19] border border-[#EDEBE9] dark:border-[#323130] rounded p-3 flex items-center justify-between text-xs">
                      <div>
          ...
          {activeSubTab === "kpi-okr" && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-[#1b1a19] p-5 border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm">
                <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase mb-3">{t("Business Development Funnel")}</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-9 gap-2 text-center">
                  {BD_PIPELINE_STAGES.map((stage) => (
                    <div key={stage} className="bg-[#FAF9F8] dark:bg-[#201f1e] border border-[#EDEBE9] dark:border-[#323130] rounded p-2">
                      <div className="text-lg font-bold text-[#0078D4]">{pipelineByStage[stage].length}</div>
                      <div className="text-[9px] text-slate-500 font-semibold mt-0.5">{t(stage)}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-[#1b1a19] p-5 border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm">
                <h3 className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase mb-3">{t("Win / Loss Analysis")}</h3>
                <div className="grid grid-cols-3 gap-3 mb-4">
                  <StatCard label={t("Won")} value={String(lossAnalysis.totalWon)} accent="text-emerald-600" />
                  <StatCard label={t("Lost")} value={String(lossAnalysis.totalLost)} accent="text-rose-600" />
                  <StatCard label={t("Win Rate")} value={`%${lossAnalysis.winRate.toFixed(0)}`} />
                </div>
                <div className="space-y-2">
                  {lossAnalysis.rows.map((row) => (
                    <div key={row.reason} className="flex items-center justify-between text-xs border-b border-[#EDEBE9] dark:border-[#323130] pb-2">
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{t(row.reason)}</span>
                      <span className="text-slate-500">
                        {row.count} (%{row.pct.toFixed(0)})
                      </span>
                    </div>
                  ))}
                  {lossAnalysis.rows.length === 0 && <p className="text-xs text-slate-400">{t("No loss data recorded yet.")}</p>}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                {showAddGoal && (
                  <form onSubmit={handleSaveGoal} className="bg-white dark:bg-[#1b1a19] border border-[#0078D4]/20 rounded p-5 shadow-md space-y-3 text-xs">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {goals.map((g) => (
                    <div key={g.id} className="bg-white dark:bg-[#1b1a19] border border-[#EDEBE9] dark:border-[#323130] rounded shadow-sm p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        <div>
