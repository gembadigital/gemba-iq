...
export default function CompaniesView() {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (importedCompanies.length === 0) {
          alert(lang === "TR" ? "Yüklenecek geçerli şirket kaydı bulunamadı. Lütfen sütun başlıklarını kontrol edin." : "No valid company records found to import. Please check column headers.");
          return;
        }
        ...
        alert(lang === "TR" ? `${importedCompanies.length} şirket başarıyla içe aktarıldı!` : `${importedCompanies.length} companies successfully imported!`);
      } catch (err: any) {
        console.error(err);
        alert(lang === "TR" ? "Dosya okunurken hata oluştu: " + err.message : "Error reading file: " + err.message);
      }
    };
  ...
  const handleExecuteBulkAction = () => {
    if (selectedCompanyIds.length === 0) {
      alert(lang === "TR" ? "Lütfen en az bir şirket seçin!" : "Please select at least one company!");
      return;
    }

    if (bulkAction === "delete") {
      if (confirm(lang === "TR" ? `Seçilen ${selectedCompanyIds.length} şirketi ve ilgili tüm verileri kalıcı olarak silmek istediğinizden emin misiniz?` : `Are you sure you want to permanently delete selected ${selectedCompanyIds.length} companies?`)) {
  ...
  const handleSaveCompanySubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formState.name) {
      alert(lang === "TR" ? "Şirket Adı zorunludur!" : "Company Name is required!");
      return;
    }
  ...
  const handleDeleteCompany = (id: string, name: string) => {
    if (confirm(lang === "TR" ? `'${name}' şirketini ve bağlı tüm satış fırsatları, teklifleri ve hakedişleri silmek istediğinizden emin misiniz?` : `Are you sure you want to delete ${name} and all associated proposals/deals?`)) {
  ...
  const handleDeleteCustomFieldDef = (id: string) => {
    if (confirm(lang === "TR" ? "Bu özel alan tanımını silmek istediğinizden emin misiniz? (Şirketlerdeki mevcut veriler kaybolacaktır)" : "Delete this custom field definition?")) {
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            <div>
              <h1 className="text-xl font-black text-slate-800 dark:text-zinc-100 flex items-center gap-2">
                ...
                <span>{lang === "TR" ? "Şirket Portföy Yönetimi" : "Client Portfolio & Accounts"}</span>
              </h1>
              <p className="text-[11px] text-slate-400 dark:text-zinc-500 mt-1 font-sans">
                {lang === "TR" 
                  ? "Tüm operasyonel, finansal, teşhis verileri ve iş geliştirme fırsatları için Tek Güvenilir Kaynak (Single Source of Truth) CRM veritabanı."
                  : "The relational single source of truth master ledger for lean manufacturing diagnostics, activities, and billing."}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 w-full md:w-auto shrink-0">
              ...
              >
                ...
                <span>{lang === "TR" ? "Alanları Özelleştir" : "Custom Fields"}</span>
              </button>
              ...
              >
                ...
                <span>{lang === "TR" ? "Yeni Şirket Ekle" : "Add Enterprise Company"}</span>
              </button>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#131313] p-4 rounded-xl border border-slate-100 dark:border-zinc-900/80 shadow-[0_1px_3px_rgba(0,0,0,0.01)] space-y-3">
            ...
            <div className="flex flex-col lg:flex-row gap-3">
              ...
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder={lang === "TR" ? "Şirket adı, sektör, şehir, temsilci ara..." : "Search corporate grid..."}
                  value={searchTerm}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2 shrink-0">
                ...
                >
                  <option value="All">{lang === "TR" ? "Tüm Sektörler" : "All Industries"}</option>
                  <option value="Manufacturing">{lang === "TR" ? "İmalat" : "Manufacturing"}</option>
                  <option value="Automotive">{lang === "TR" ? "Otomotiv" : "Automotive"}</option>
                  <option value="Textiles">{lang === "TR" ? "Tekstil" : "Textiles"}</option>
                </select>
                ...
                >
                  ...
                  <option value="Lead">{lang === "TR" ? "Aday" : "Lead"}</option>
                  <option value="Prospect">{lang === "TR" ? "Potansiyel" : "Prospect"}</option>
                  <option value="Active Customer">{lang === "TR" ? "Aktif Müşteri" : "Active"}</option>
                </select>
                ...
                >
                  <option value="All">{lang === "TR" ? "Tüm Sorumlular" : "All Owners"}</option>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-t border-dashed border-slate-100 dark:border-zinc-800 pt-3">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <span className="text-[10px] text-slate-400 font-mono shrink-0">
                  {selectedCompanyIds.length} {lang === "TR" ? "Şirket Seçili" : "Selected"}
                </span>
                ...
                >
                  <option value="">{lang === "TR" ? "-- Toplu İşlem Seçin --" : "-- Choose Bulk Action --"}</option>
                  <option value="delete">{lang === "TR" ? "Seçilenleri Sil" : "Delete Selected"}</option>
                  <option value="status:Active Customer">{lang === "TR" ? "Durumu: Aktif Müşteri Yap" : "Set Status: Active"}</option>
                  <option value="status:Prospect">{lang === "TR" ? "Durumu: Potansiyel Yap" : "Set Status: Prospect"}</option>
                  <option value="owner:Atakan Zehir">{lang === "TR" ? "Sorumlu: Atakan Zehir Yap" : "Assign Owner: Atakan"}</option>
                </select>
              ...
              <div className="flex flex-wrap items-center gap-2 shrink-0 ml-auto w-full sm:w-auto justify-end">
                ...
                <button
                  ...
                  title={lang === "TR" ? "Geniş Görünüm" : "Toggle Full-Width Grid"}
                >
                ...
                <div className="inline-flex rounded-lg border border-slate-205 dark:border-zinc-800 overflow-hidden">
                  <button
                    ...
                    title={lang === "TR" ? "CSV Dosyasından İçe Aktar" : "Import from CSV File"}
                  >
                  ...
                  <button
                    ...
                    title={lang === "TR" ? "Excel Dosyasından İçe Aktar" : "Import from Excel File"}
                  >
                    <Upload className="w-3.5 h-3.5 text-indigo-550" />
                    <span>EXCEL</span>
                  </button>
                  ...
                  {paginatedCompanies.map((c) => (
                    <tr key={c.id} className="hover:bg-slate-50/50 dark:hover:bg-zinc-900/30 transition-colors">
                      ...
                      <td className="p-3 text-slate-600 dark:text-zinc-400">
                        {c.billingCity ? `${c.billingCity}, ${c.billingDistrict || ""}` : (lang === "TR" ? "Bursa" : "Bursa, TR")}
                      </td>
                      ...
                      <td className="p-3 text-right pr-5">
                        <div className="inline-flex items-center gap-1">
                          <button
                            ...
                            title={lang === "TR" ? "Düzenle" : "Edit Profile"}
                          >
                          ...
                          <button
                            ...
                            title={lang === "TR" ? "Kalıcı Olarak Sil" : "Delete Company"}
                          >
            ...
            {totalPages > 1 && (
              <div className="p-3.5 border-t border-slate-100 dark:border-zinc-850/80 bg-slate-50 dark:bg-zinc-900/40 flex items-center justify-between">
                <span className="text-[10px] font-mono text-slate-450 dark:text-zinc-500">
                  {lang === "TR" 
                    ? `Toplam ${filteredCompanies.length} şirketten ${(currentPage - 1) * itemsPerPage + 1}-${Math.min(currentPage * itemsPerPage, filteredCompanies.length)} arası gösteriliyor`
          ...
          >
            ...
            <div className="p-4 bg-slate-50 dark:bg-zinc-900 border-b border-slate-100 dark:border-zinc-800/80 flex justify-between items-center">
              <span className="font-extrabold text-slate-800 dark:text-zinc-150 text-sm flex items-center gap-2">
                ...
                {editingCompany 
                  ? (lang === "TR" ? `Profili Düzenle: ${editingCompany.name}` : `Edit Account Profile: ${editingCompany.name}`)
                  : (lang === "TR" ? "Yeni Şirket Kaydı Oluştur" : "Create Enterprise Client Record")}
              </span>
              <button
                type="button"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1">{lang === "TR" ? "Şirket Adı *" : "Company Name *"}</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1">
                    {lang === "TR" ? "Müşteri Durumu" : "Customer Status"}
                  </label>
                  <select
                    ...
                    className="w-full bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded p-1.5 focus:outline-none"
                  >
                    <option value="Lead">{lang === "TR" ? "Aday Müşteri (Lead)" : "Lead"}</option>
                    <option value="Prospect">{lang === "TR" ? "Potansiyel Müşteri (Prospect)" : "Prospect"}</option>
                    <option value="Active Customer">{lang === "TR" ? "Aktif Müşteri (Active)" : "Active Customer"}</option>
                    <option value="Inactive">{lang === "TR" ? "Pasif (Inactive)" : "Inactive"}</option>
                  </select>
                </div>

                <div>
                  <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1">{lang === "TR" ? "Telefon" : "Phone"}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1">{lang === "TR" ? "Web Sitesi" : "Web Site"}</label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded p-1.5 focus:outline-none"
                  >
                    <option value="Manufacturing">{lang === "TR" ? "İmalat (Manufacturing)" : "Manufacturing"}</option>
                    <option value="Automotive">{lang === "TR" ? "Otomotiv (Automotive)" : "Automotive"}</option>
                    <option value="Textiles">{lang === "TR" ? "Tekstil (Textiles)" : "Textiles"}</option>
                    <option value="Technology / Software">{lang === "TR" ? "Teknoloji / Yazılım (Technology / Software)" : "Technology / Software"}</option>
                ...
                {(formState.industry === "Other" || formState.industry === "Diğer" || formState.industry === "diğer") && (
                  <div className="col-span-1 md:col-span-2">
                    <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1">
                      {lang === "TR" ? "Sektör Detayı / Açıklama *" : "Custom Industry Explanation *"}
                    </label>
                    <input
                      type="text"
                      required
                      ...
                      placeholder={lang === "TR" ? "Örn: Savunma Sanayi, Gıda, İlaç..." : "e.g. Defense, Food, Pharma..."}
                ...
                <div>
                  <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1">
                    {lang === "TR" ? "Hesap Temsilcisi" : "Account Owner"}
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[9px] uppercase font-bold text-slate-400 mb-1">
                    {lang === "TR" ? "Vergi Numarası" : "Tax Number"}
                  </label>
                  <input
                    type="text"
            ...
            <div className="p-4 bg-slate-50 dark:bg-zinc-900 border-t border-slate-100 dark:border-zinc-800/80 flex justify-end gap-2">
              ...
              >
                {lang === "TR" ? "Kaydet" : "Confirm Profile"}
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-[#121212] border border-slate-205 dark:border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl animate-scaleIn text-xs">
            ...
            <div className="p-4 bg-slate-50 dark:bg-zinc-900 border-b border-slate-100 dark:border-zinc-800 flex justify-between items-center">
              <span className="font-bold text-slate-800 dark:text-zinc-200 text-sm flex items-center gap-2">
                ...
                {lang === "TR" ? "Özel Kurumsal Alan Tanımları" : "Manage Custom CRM Fields"}
              </span>
              <button
                type="button"
            ...
            <div className="p-5 space-y-4">
              ...
              <form onSubmit={handleAddCustomFieldDef} className="bg-slate-50 dark:bg-zinc-900 p-3.5 border border-slate-150 dark:border-zinc-850 rounded-xl space-y-3">
                <span className="font-bold text-[10px] uppercase font-mono tracking-wider block text-slate-400">Add custom column</span>
                
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[8px] uppercase font-bold text-slate-400 mb-1">Field Label *</label>
                    <input
                      type="text"
                      required
                      placeholder="e.g. ERP Vendor, ISO Cert"
                  ...
                  <div>
                    <label className="block text-[8px] uppercase font-bold text-slate-400 mb-1">Type</label>
                    <select
                ...
                >
                  {lang === "TR" ? "Yeni Alan Tanımını Ekle" : "Add Custom Field"}
                </button>
              </form>

              {/* List of current custom definitions */}
              <div className="space-y-2">
                <span className="font-bold text-[10px] uppercase font-mono tracking-wider block text-slate-400">Current dynamic definitions</span>
                
                {customFieldDefs.length === 0 ? (
                  <p className="text-slate-400 italic text-center py-4">No custom fields defined yet.</p>
                ) : (
            ...
            <div className="p-4 bg-slate-50 dark:bg-zinc-900 border-t border-slate-100 dark:border-zinc-800 flex justify-end">
              <button
                ...
                className="px-4 py-1.5 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-lg font-semibold"
              >
                {lang === "TR" ? "Kapat" : "Close Customizer"}
              </button>
            </div>
