                ...
                <div key={item.id} className="relative group/timeline animate-fadeIn text-xs font-sans">
                  ...
                  <div className="space-y-1">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1">
                      ...
                      {/* Kullanıcı talebi: "Zaman tüneli giriş bilgileri
                          notları fontlar çok küçük, en az 9 font olsun ve
                          bir tık koyu font rengi kullan." — tarih/saat
                          metası büyütüldü (10px -> 11px) ve bir ton
                          koyulaştırıldı (slate-450 -> slate-550). */}
                      <span className="text-[11px] text-slate-550 dark:text-zinc-400 font-mono flex items-center gap-1 shrink-0">
                    ...
                    <p className="text-slate-750 dark:text-zinc-200 font-sans leading-relaxed whitespace-pre-wrap max-w-3xl pr-1 text-[13px]">
                      {item.description}
                    </p>

                    {item.result && (
                      <div className="inline-flex items-center gap-1.5 px-2 py-0.5 mt-1 rounded bg-emerald-50 dark:bg-emerald-950/20 text-[11px] font-mono text-emerald-700 dark:text-emerald-400 border border-emerald-500/10">
                        <CheckCircle className="w-3 h-3" />
                        <span><b>{t("Outcome:")}</b> {item.result}</span>
                      </div>
                    )}

                    {/* "Logged By" metası: 9px -> 10px, en açık renk
                        (slate-400/zinc-500) -> bir tık koyu (slate-500/zinc-400). */}
                    <div className="text-[10px] text-slate-500 dark:text-zinc-400 font-mono mt-1.5">
                      {t("Logged By:")} <span className="font-bold text-slate-600 dark:text-zinc-300">{item.user}</span>
                    </div>
                  </div>
